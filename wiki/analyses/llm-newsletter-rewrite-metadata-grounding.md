...
## 주간 다이제스트 변형 — 일일의 「압축」 모드

일일 rewrite 룰 (룰 1~10) 은 *오늘 도착한 후보* 를 한 페이지로 정리하는 규칙이지만, **주간 다이제스트** 는 지난 7일치 일일 출력 JSON 배열을 입력받아 *한 주의 흐름* 을 한 페이지로 압축하는 별개의 변형이 필요하다. dev-blog 의 `weekly-linux.mjs` 가 daily 흐름 안에서 같이 호출되며 (월요일 새 ISO 주가 바뀔 때 새 파일 생성), prompt 본문에서 새로 강조되는 룰은 다음 4가지:

### 주간 룰 W1 — 같은 흐름은 묶기 (진행 흐름 한 줄)

한 시리즈/이슈가 *여러 날 걸쳐* 등장했다면 그날그날의 highlight 를 그대로 옮기지 말고 **진행 흐름 한 줄** 로 묶는다. 일일 룰 2 (`history` 의 시간축 처리) 의 *주 단위 확장* 으로, 일주일 안에서 일어난 상태 전이를 한 항목으로 응축:

- 좋은 예: "ACRN irqfd 시리즈 — 월: v2 등장 → 수: 메인테이너 피드백 (cleanup 순서 지적) → 금: v3 보류"
- 좋은 예: "6.19.14 stable EOL 백포트 — 화: changelog 공개 (219건) → 목: bcache/KASAN UAF 추가 백포트 → 금: EOL 직전 마지막 묶음"
- 나쁜 예: 일일 highlight 의 title 을 그대로 7개 나열 → 독자가 흐름을 재구성해야 함

### 주간 룰 W2 — 이번 주의 3~5개 흐름만

일일 룰 9 (priority 분포) 가 「하루 4개」 였다면 주간은 **「한 주 3~5개 흐름」**. 7일치 highlights 를 단순 union 하면 정보 과잉. 일일 4개 × 7일 = 28개 후보 중 3~5개만 본문, 나머지는 "기타" 한 줄로 묶거나 통째 생략. 「7일 평균치」 가 아니라 「7일 중 가장 중요한 3~5건」 — 평탄화 금지가 핵심.

### 주간 룰 W3 — 국부 드라이버 제외 (일일 정책 그대로)

일일 룰 8 (국부 드라이버는 본문에서 제외) 가 그대로 적용. 일주일 동안 누적된 단일 칩·보드·드라이버 모듈 패치는 본문에서 통째 생략하거나 「기타」 한 줄. 일일에서 솔직히 빠진 것을 「주간이니까 보충」 의 함정에 빠지지 말 것.

### 주간 룰 W4 — dailies 배열 재포함 금지

입력으로 받은 일일 JSON 배열 (`dailies`) 은 *참고 자료* 일 뿐, 출력 JSON 에 다시 포함하지 않는다. 일일 룰 10 (`candidateBodies` 재포함 금지) 의 주간판. 멱등성 + 토큰 절약 동시 해결.

### 일반 사상

주간 다이제스트는 **일일의 합집합이 아니라 일일의 압축**. 일주일이라는 시간 축이 추가됐으니 시간축 위의 *상태 전이* 를 본문 단위로 끌어올리는 게 본질. 일일 룰의 「하루치」 단위를 「한 주치」 로 그대로 늘리면 정보 과잉으로 자멸한다.
...
| Android 커널 | 동일 + ACK prefix 풀어쓰기 (ANDROID/FROMGIT/FROMLIST/BACKPORT/UPSTREAM) | 동일 + ACK 브랜치 진행 (android-mainline / android16-6.12) | Android 메인테이너 (linux-android) | 동일 |
| OSS Trending | README excerpt + description + topics + ageDays/pushAgeDays + hn 메타 | star velocity 기록, HN 등장일, 신규 (60일 내 created) vs 활발 (별 5k+, 7일 내 push) 분리 | 코어 메인테이너 commit 비율 | HN 스레드 / GitHub PR 응답 |
| OSS Curation | description + topics + `analysisExcerpt` (`hasAnalysis=true` 일 때) + `curationScore` + readmeExcerpt | 큐레이션 사이클 (주간) 내 재선정 여부 | 코어 유지자·재단 발표 | 분석 발췌의 톤 (도입 권장 / 주의 / 비교) |
| Linux Lens (보안·도구체인·RT·GPU·관측성 등) | linux-newsletter 와 동일 + 렌즈 메일링 리스트 컨텍스트 | linux-newsletter 와 동일 + `draftMetadata.signalLevel` ([[llm-content-quality-guards]] 가드 4 참조) | 동일 | 동일 |
...
### Android 커널 의 ACK prefix 풀어쓰기 룰

Android 커널 토픽 (ACK = Android Common Kernel) 의 commit message 는 prefix 가 그 패치의 *유형 신호* 다. LLM 은 prefix 를 그대로 옮기지 말고 한국어로 풀어 설명:

| prefix | 의미 | 본문 톤 |
|--------|------|---------|
| `ANDROID:` | Android 전용 패치 (mainline 에 없음) | vendor·플랫폼 영향 *가장 큼*, GKI/ABI 변경 시 priority 「상」 |
| `FROMGIT:` | upstream maintainer tree 에서 미리 가져온 패치 | mainline 머지 임박 신호 |
| `FROMLIST:` | LKML 리뷰 중인 패치를 ACK 가 조기 적용 | 검토 진행 중, ABI 영향 미확정 |
| `BACKPORT:` | mainline 에서 ACK 로 백포트 | 안정성·보안 강화 |
| `UPSTREAM:` | mainline 이미 머지된 패치 | 대개 merge commit, ABI 변경 없는 정리는 「하」 |

본문 좋은 예: 「ANDROID 전용 BPF helper 추가로 vendor module 빌드 시 KMI ABI 검사를 한 번 더 돌려야 합니다」 — prefix 의 *의미* 가 본문 첫 절에 자연스럽게 녹아 있음. 나쁜 예: 「`ANDROID:` 패치입니다. ANDROID 는 …」 (prefix 를 메타설명).
...
- 2026-05-18 (4th): 같은 launchd 사이클의 Linux Kernel Weekly Digest 프롬프트 (07:30 발사) 본문에서 「주간 다이제스트 변형」 섹션 신설 — 주간 룰 W1~W4 (같은 흐름 묶기 / 3~5개 흐름만 / 국부 드라이버 제외 / dailies 재포함 금지). 주간은 일일의 합집합이 아니라 압축이라는 일반 사상 정리. 일일 룰 (2/8/9/10) 의 시간축 확장 버전. 추가로 같은 사이클의 Linux Lens 4종 (07:19/07:23/07:25/07:29) 은 lens 토픽만 다른 동일 prompt body 라 신규 룰 흡수 없이 sources 만 갱신 가능 — 본 페이지에서는 source 누적이 의미 없어 생략 (참고: [[dev-blog]] 변경 이력에는 포함). (출처: session-logs/20260518-073052-9857-*.md)
