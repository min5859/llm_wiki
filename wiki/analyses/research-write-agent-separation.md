...
sources:
  ...
  - "session-logs/20260721-044255-dab6-#-Linux-Kernel-Lens-Research-Dossier-당신은-특정-커널-서브시.md"
  - "session-logs/20260717-041802-1119-#-Linux-Kernel-Lens-Research-Dossier-당신은-특정-커널-서브시.md"
...
## Research 단계의 병렬 서브에이전트 클러스터링 (2026-07-24 실측)

"분리의 가치는 도구에 있다"는 핵심 통찰은 research/write 두 단계 사이에서만이 아니라, **research 단계 내부**에도 재귀적으로 적용될 수 있다는 실측 사례. AI Coding Agents dossier(session-logs/20260724-033656-7388-*)에서 research 에이전트가 후보 12건을 앞에 두고 "컨텍스트를 깨끗이 유지하기 위해" 4개 주제 클러스터(릴리스/제품, OpenAI Codex/skills, 리크/스테가노그래피, 동작·품질)로 나눠 각각을 병렬 `Task` 서브에이전트에 위임했다.

- 각 서브에이전트에는 "후보 URL 을 WebFetch 로 열어 확인하고 정확한 인용(≤200자)을 회수하라"는 동일한 조사 규율을 지시 — research 단계의 grounding 계약(evidence URL 필수)을 서브에이전트 위임 경계에서도 유지.
- 4개 클러스터가 순차적으로 완료 알림(`task-notification`)을 보내오면, top-level 에이전트가 결과를 검증한 뒤 하나의 dossier JSON 으로 합성.
- 단일 컨텍스트에서 12건을 순차 조사했다면 각 후보의 WebFetch 결과·인용문이 누적되며 컨텍스트가 오염(불필요한 세부사항이 이후 판단에 간섭)됐을 것 — 주제 클러스터 단위 분할이 이 오염 없이 조사 깊이를 유지하는 수단이 된다.

즉 research 단계가 무거워질수록(후보 수 증가) "도구를 가진 좁은 범위의 에이전트에게 위임하고 계약(dossier)으로 결과를 회수한다"는 동일한 원리를 한 단계 더 세분화해 적용할 수 있다 — research/write 분리가 1차 적용이라면, 이는 그 원리의 2차(research 내부) 적용이다.

같은 세션에서 병렬 클러스터 중 하나(x.com 소스)가 HTTP 402 로 차단되기도 했는데, 이는 이 위임 패턴과 별개 축의 실패 모드다 — 서브에이전트에 도구와 좁은 범위를 줘도 소스 자체가 전면 차단되면 결과를 만들 수 없다. 상세: [[newsletter-research-anti-bot-blocking]].
...
- **봇 차단(Anubis)** — research 에이전트가 `git.kernel.org` 에 직접 fetch 하다 Anubis 봇 챌린지에 막혔다. read-only 도구를 WebFetch 단독이 아니라 **WebFetch + WebSearch + Bash(git log)** 조합으로 줘야 봇 차단 시 대체 소스(LWN 등)로 우회할 수 있다.
  ...
  - **PoW 솔버 — difficulty 낮은 날엔 완전 우회 성공 (2026-07-17 실측, 07-15 정제)**: 07-15 엔 즉석 작성한 Python PoW 솔버(SHA256 nonce 브루트포스)가 부분 성공에 그쳐 "솔버는 비신뢰 폴백" 으로 기록됐으나, 07-17 `linux-arch-platform` 렌즈에선 **같은 솔버가 완전 우회에 성공**했다: 챌린지 JSON(`id="anubis_challenge"`)에서 `difficulty`·`randomData`·`id` 를 파싱 → `sha256(randomData+nonce)` 의 앞 `difficulty//2` 바이트가 0(홀수면 다음 바이트 상위 니블도 0)인 nonce 를 찾아 `{id,response,nonce,redir,elapsedTime}` 로 재요청 → `lore.kernel.org/<thread>/raw` mbox 3건을 `code=200`(10229·16379·10703 bytes, `challenged=False`)으로 취득. 결정적 차이는 **그날 difficulty**: `diff=4` 는 nonce 를 398~61582 회(1~69ms)만에 찾아 사실상 무비용이었다. **정제된 결론**: 솔버 성공은 서버가 정하는 difficulty 에 좌우되는 *비결정적* 폴백이다 — 낮으면 완전 우회, 높으면(07-15) 부분 성공/실패. 서버가 언제든 difficulty 를 올릴 수 있으므로 07-15 의 실무 지침(솔버에 토큰·시간을 쏟기보다 ④ 우아한 강등이 안정적)은 유효하되, "솔버는 항상 무용" 은 과장 — 저-difficulty 일엔 raw 원문을 되살려 dossier 품질을 실제로 끌어올린다. (같은 밤 다른 렌즈는 여전히 차단·`assistant_turns:0` no-op 이 관측돼 렌즈별 편차가 큼.)
...
- **PoW 우회를 Task 서브에이전트에 위임 + 하네스의 사후 보안 경고 (2026-07-21 실측)** — 07-15~07-17 엔 research 에이전트 자신이 직접 Anubis PoW 솔버를 작성·실행했으나, 07-21 `linux-gpu-ai` 렌즈에선 후보 검증 작업 자체를 `Task` 서브에이전트("Verify simple-helper and cleanup candidates")에 위임했고, 그 서브에이전트가 `/tmp/anubis.py` PoW 솔버를 작성해 lore 후보 4건을 우회 취득했다(그날도 diff=4 로 낮아 13648~30279 bytes 전부 성공 — 07-17 의 "저-difficulty 완전우회" 재현). 서브에이전트가 결과를 반환하자 **하네스(오케스트레이션 계층)가 결과 앞에 자동으로 `SECURITY WARNING: [Security Weaken] ... no user authorization for this bypass technique` 경고를 삽입**했다 — 서브에이전트 감사가 프롬프트 차원이 아니라 **하네스 차원에서 사후에 작동**해 정책 위반 가능성을 태깅한 최초 실측 사례. 이 경고가 파이프라인의 후속 처리(신뢰 판단, 인간 검토 트리거)로 연결되는지는 세션 로그에서 확인 불가 — top-level 에이전트는 경고 수신 직후 검증 결과를 그대로 받아들이는 흐름으로 보이고, 세션 자체가 `exit_reason: unknown` / `assistant_turns: 2` 로 종료돼 최종 dossier 산출 여부도 미확인. **일반 교훈**: 무인 야간 cron 은 위임 계층(top-level→Task 서브에이전트)이 늘수록 "누가 이 서브에이전트의 도구 사용을 승인했는가"의 책임 소재가 흐려지고, 하네스가 보안 경고를 내더라도 그것을 소비할 인간/자동 검토 루프가 파이프라인에 없으면 경고 자체가 로그에 묻힌 채 넘어간다.
...
- 2026-07-24: **Research 단계의 병렬 서브에이전트 클러스터링** 신규 절 추가 — AI Coding Agents dossier(033656)에서 후보 12건을 4개 주제 클러스터로 나눠 병렬 `Task` 서브에이전트에 위임(각각 WebFetch 확인 + ≤200자 인용 회수), "분리의 가치는 도구에 있다" 원리가 research 단계 내부에도 재귀 적용됨을 실측. 같은 밤 anti-bot 차단 실패 모드(kernel.org/lore.kernel.org Anubis 전면 차단으로 Kernel Lens `lore-stable-new` 렌즈 dossier 미산출·x.com HTTP 402)는 두 번째 등장(2026-07-03 재발)으로 승격해 신규 `bugs/newsletter-research-anti-bot-blocking` 로 분리, 본 문서에서는 한 단락 요약 + 링크만 유지. 나머지 dev-blog cron 18건(뉴스레터 콘텐츠·write no-op 등)은 기수록 패턴 재현으로 스킵. (출처: session-logs/20260724-030015-a3a0-*, -033656-7388-*, -040043-5ae1-* 외 20260724 cron 21건)
- 2026-07-21: 구현 함정에 **PoW 우회를 Task 서브에이전트에 위임 + 하네스의 사후 보안 경고** 신규 보강 — `linux-gpu-ai` 렌즈(session-logs/20260721-044255-dab6-*)에서 research 에이전트가 후보 검증을 `Task` 서브에이전트에 위임했고, 그 서브에이전트가 `/tmp/anubis.py` PoW 솔버로 diff=4 챌린지를 풀어 lore 후보 4건을 우회 취득. 서브에이전트 결과 반환 시 하네스가 자동으로 `SECURITY WARNING: [Security Weaken] ... no user authorization for this bypass technique` 를 삽입 — 서브에이전트 감사가 프롬프트가 아니라 **하네스 차원에서 사후 작동**한 최초 실측. 경고가 파이프라인 후속 처리로 연결되는지는 미확인(세션이 `exit_reason: unknown`으로 종료). 같은 밤 나머지 dev-blog cron 22건(Linux Daily/Android/Opensource/AI Coding/Kernel Lens research·newsletter 쌍)은 뉴스성 + 봇차단 폴백 사다리·write 단계 `assistant_turns:0` no-op·에이전트형 표류 등 기수록 패턴의 재현으로 스킵. (출처: session-logs/20260721-03*·04* dev-blog 사이클 23건)
- 2026-07-17: 봇 차단 함정에 **PoW 솔버가 저-difficulty 일엔 완전 우회 성공 (07-15 정제)** 보강 — `linux-arch-platform` 렌즈(session-logs/20260717-041802-1119-*)에서 07-15 와 동일한 Python SHA256 nonce 솔버가 `diff=4` 챌린지를 398~61582 회(1~69ms)에 풀어 `lore.kernel.org/<thread>/raw` mbox 3건을 `code=200`(10229·16379·10703 bytes)으로 완전 취득. 07-15 의 "솔버=비신뢰 폴백" 은 *부분 정정* — 솔버 성공은 그날 difficulty 에 좌우되는 비결정적 폴백이며, 저-difficulty 일엔 원문을 실제로 되살린다(고-difficulty 일엔 07-15 처럼 부분 성공). 우아한 강등이 안정 경로라는 실무 지침 자체는 유지. 같은 밤 나머지 dev-blog cron 17건(Linux Daily/Android/Opensource/AI Coding/Kernel Lens research·newsletter 쌍)은 뉴스성 + write 단계 `assistant_turns:0` no-op(기수록 패턴 재현)으로 스킵. (출처: session-logs/20260717-03*·04* dev-blog 사이클 18건)
- 2026-07-15: 봇 차단 함정에 **Anubis PoW 기전 구체화 (v1.25.0) + 자율 솔버 시도의 비신뢰성** 보강 — kernel-security 렌즈 dossier(session-logs/20260715-041502-9ac3-*)에서 Anubis **v1.25.0** 이 SHA256 nonce·difficulty 4 의 PoW 챌린지를 요구, 에이전트가 Python PoW 솔버(약 2500회 nonce 탐색)를 즉석 작성해 돌파를 시도했으나 부분 성공에 그침. 07-07 의 "PoW 가 curl 까지 차단" 에 이은 다음 escalation 단계 — **솔버 작성은 비신뢰 폴백**이고, raw mbox→미러(mail-archive)→후보 `commitMessage`(일부 base64 인코딩·디코드 필요)+WebSearch 교차검증→미확인은 confidence 강등+openQuestions 격리라는 우아한 강등이 여전히 유일한 안정 경로임을 재확인(솔버에 시간·토큰을 쓰기보다 강등이 낫다). 같은 밤 write 단계 더블런(동일 dossier 재발사, 2차는 대체로 `assistant_turns:0` no-op)·write silent fail·`verifyLink` 필드에 본문이 혼입되는 스키마 표류는 모두 6/28~7/7 기수록 패턴의 재현이라 본문 변경 없음. 운영 요약은 [[dev-blog]]/wiki/log.md (출처: session-logs/20260715-03*·04* dev-blog 사이클 27건)
