...
- **단점**:
  ...
  - **버전 동기화 필수** — 에이전트 내부 변경에 UI 가 깨지므로 같은 릴리스로 함께 업그레이드해야 한다. 게다가 파일을 갱신해도 **실행 중 프로세스를 재시작하지 않으면** UI(신)와 메모리 상 agent(구)의 버전이 어긋나 런타임 AttributeError 가 난다 → 실제 발현 사례 [[stale-process-attributeerror-inprocess-coupling]].
  - 원격 분리 배포가 어렵다.
