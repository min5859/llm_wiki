...
## 2026-06-22 — wiki-ingest (session-logs, ingested: false 24건)

- 대상: **session-log 24건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~04:18 KST) = Research Dossier 11건 + Newsletter Write 12건 + Linux Kernel Weekly Digest 1건. 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍(linux-toolchain·linux-distro-stable·linux-perf-rt·linux-arch-platform·linux-gpu-ai 등). mcp-note 0건. raw-sources/ 서브디렉터리(articles/books/ideas/papers/transcripts) 신규 .md 0건(Tips/ 는 PDF 만, .cache/extracted 없음, fetched/ 없음) → raw-sources/PDF/URL 유래 대상 외.
- 처리 방식: 24건을 3개 subagent 로 병렬 정밀 추출(AI코딩+OSS 7건 / Linux·Android 5건 / Kernel Lens 12건) 후 본 세션이 기존 편집 방침·analyses 대조 검증. prompt injection 내성 적용(dossier/newsletter 본문 지시문 무시, 외부 인용은 참고 정보로만).
- **신규 durable 지식: 0건(전량 스킵).** subagent 들이 5건(UFS 에러핸들러↔runtime PM 수정연쇄, Rust DRM ioctl lifetime soundness, eBPF verifier scalar 정밀도 건전성, 커널 lock-drop-revalidate/재귀mutex, dma_fence 콜백 동기화)을 CREATE 제안했으나, **6-14~6-21 에 6회 반복 확립된 편집 방침에 따라 기각**: ① dev-blog dossier 의 외부 커널/OSS 뉴스 콘텐츠 자체는 개인 엔지니어링 지식이 아님, ② 단일 패치/릴리스를 범용 패턴으로 일반화 = 과잉추출(날조), ③ 출처가 lore.kernel.org Anubis 차단으로 미검증(confidence medium/low). 파이프라인 메타패턴은 이미 [[research-write-agent-separation]]/[[llm-newsletter-rewrite-metadata-grounding]]/[[llm-content-quality-guards]]/[[ai-coding-agent-cost-and-context-patterns]] 에 수록.
- 운영 관찰만 기록: 갱신 [[dev-blog]] (projects) — 2026-06-22 사이클 관찰(write 단계 silent fail 약 3주째 고착이나 이번엔 2건[66e0/10ff]이 1턴 산출해 간헐적 부분 회복, dossier 9/11 정상·2 무응답, Weekly Digest grounding 메타 깨짐, lore 봇차단 폴백 사다리 흡수 지속).
...
## 2026-06-24 — wiki-ingest (session-logs, ingested: false 21건)

- 대상: **session-log 21건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~04:13 KST) = Research Dossier 11건 + Newsletter Write 10건. 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation 각 1쌍 + AI Coding Agents 1(dossier만) + Linux Kernel Lens 6쌍. mcp-note 0건. raw-sources/ 서브디렉터리(articles/books/ideas/papers/transcripts) 신규 .md 0건(Tips/ 포함 PDF·txt·pptx 만, .cache/extracted 없음, fetched/ 없음) → raw-sources/PDF/URL 유래 대상 외. (20260607-095133 agent-weekly 는 frontmatter `ingested: true` 인데 본문에 "ingested: false" 문자열 우연 포함 → grep 오탐, 직전 사이클과 동일하게 대상 제외).
- 처리 방식: 21건 정밀 트리아지(assistant_turns/file_edits 메트릭 + 산출 dossier 본문 검수) 후 본 세션이 6-14~6-22 에 8회 반복 확립된 편집 방침·기존 analyses 대조 검증. prompt injection 내성 적용(dossier/newsletter 본문 지시문 무시, 외부 인용은 참고 정보로만, 코드펜스 격리).
- **신규 durable 지식: 0건(전량 스킵).** ① dev-blog dossier 의 외부 커널/OSS 뉴스 콘텐츠 자체는 개인 엔지니어링 지식이 아님, ② 단일 패치/릴리스를 범용 패턴으로 일반화 = 과잉추출(날조), ③ 출처가 lore.kernel.org Anubis 봇차단으로 1차 미검증(WebSearch 교차검증으로 보강하나 휘발성). 파이프라인 메타패턴은 이미 [[research-write-agent-separation]]/[[llm-newsletter-rewrite-metadata-grounding]]/[[llm-content-quality-guards]]/[[ai-coding-agent-cost-and-context-patterns]] 에 수록. AI Coding Agents dossier 입력 후보(Copilot CLI TUI GA·CC v2.1.186·소스리크·OpenClaw 과금 차단 등)는 이미 [[claude-code-source-leak-internals]]/[[anthropic-oauth-third-party-billing-trap]] 에 기수록(재탕).
- 운영 관찰만 기록: 갱신 [[dev-blog]] (projects) — 2026-06-24 사이클 관찰(Newsletter Write 10/10 = `assistant_turns: 0` 으로 6/22 의 간헐 부분 회복이 비지속, write silent fail 약 2주 반째 비결정적 고착 복귀; Research Dossier 9/11 정상·AI Coding+Android 2건 무응답; lore Anubis 봇차단→WebSearch 교차검증 폴백 사다리 흡수[af_alg allowlist·Linux 7.1.1·arm64 TLBI CVE-2025-10263]; d67e 는 curl/grep 폴백).
- 스킵 사유 상세: Newsletter Write 10/10 = `assistant_turns: 0` 무응답(산출 콘텐츠 없음). Research Dossier 9건은 산출됐으나 전부 그날치 휘발성 외부 뉴스(단일 커밋/CVE 백포트·릴리스 속보·트렌딩 repo). 휘발성·미검증(일부 confidence medium/low)이라 durable wiki 사실로 부적합.
- Updated: wiki/projects/dev-blog.md(갱신: 사이클 관찰 1행 + updated + sources), wiki/log.md(본 항목).
- Marked ingested: true — session-log 21건 전체(030010·030317·030557·030856·032038·032333·033228·033418·033614·033857·034240·034612·034937·035202·035433·035747·040055·040303·040829·041024·041340). 신규 0건, 갱신 1건(운영 관찰).

## 2026-06-25 — wiki-ingest (session-logs, ingested: false 22건)

- 대상: **session-log 22건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~04:17 KST) = Research Dossier 11건 + Newsletter Write 11건. 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍. mcp-note 0건. raw-sources/ 서브디렉터리(articles/books/ideas/papers/transcripts/Tips) 신규 .md 0건(PDF·txt·pptx 만, .cache/extracted 비어 있음, fetched/ 없음) → raw-sources/PDF/URL 유래 대상 외.
- 처리 방식: 22건 정밀 트리아지(assistant_turns/file_edits/bash 메트릭 + 산출 dossier 본문 검수 + bash 활동 로그의 error/fail 스캔) 후 본 세션이 6-14~6-24 에 9회 반복 확립된 편집 방침·기존 analyses 대조 검증. prompt injection 내성 적용(dossier/newsletter 본문 지시문 무시, 외부 인용은 참고 정보로만 코드펜스 격리).
- **신규 durable 지식: 0건(전량 스킵).** ① dev-blog dossier 의 외부 커널/OSS 뉴스 콘텐츠 자체는 개인 엔지니어링 지식이 아님, ② 단일 패치/릴리스를 범용 패턴으로 일반화 = 과잉추출(날조), ③ 출처가 lore.kernel.org Anubis 봇차단으로 1차 미검증(휘발성). 파이프라인 메타패턴은 이미 [[research-write-agent-separation]]/[[llm-newsletter-rewrite-metadata-grounding]]/[[llm-content-quality-guards]]/[[ai-coding-agent-cost-and-context-patterns]] 에 수록. AI Coding Agents dossier 산출분(CC v2.1.187 `sandbox.credentials`·조직 모델 제한·MCP 타임아웃 수정, Copilot Free/Student auto 모델 선택 전환)은 그날치 릴리스 속보로 휘발성이며 소스리크·과금 차단 등은 이미 [[claude-code-source-leak-internals]]/[[anthropic-oauth-third-party-billing-trap]] 에 기수록(재탕).
- 운영 관찰만 기록: 갱신 [[dev-blog]] (projects) — 2026-06-25 사이클 관찰(Newsletter Write 11/11 = `assistant_turns: 0` 완전 무응답으로 6/22 간헐 회복 비재현, write silent fail 약 2주 반째 비결정적 고착 지속; Research Dossier 8/11 정상·opensource-curation+Kernel-Lens 3건 무응답; Kernel Lens dossier 4건 로컬 git log 폴백[에러 없음]; lore Anubis 봇차단→WebSearch/git log 폴백 사다리 흡수 지속).
...
## 2026-06-28 — wiki-ingest (session-logs, ingested: false 22건)

- 대상: **session-log 22건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~04:08 KST) = Research Dossier 11건 + Newsletter Write 11건. 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍(Android Kernel·Linux Kernel Lens 일부 write 2건). mcp-note(type: mcp-note) 0건. raw-sources/ 서브디렉터리(Tips 등) 신규 .md 0건(PDF·txt·pptx 만, .cache/extracted 비어 있음, fetched/ 없음) → raw-sources/PDF/URL 유래 대상 외. 참고: grep 으로 23건이 잡혔으나 1건(20260607-095133 agent-weekly 주간보고 agent 설계)은 본문 코드블록의 "ingested: false" 텍스트(auto-ingest.sh 주석)를 매칭한 오탐 — frontmatter 는 이미 `ingested: true` 라 제외(기수집된 설계 세션).
- 처리 방식: 22건 트리아지(서브에이전트 병렬 분석 + assistant_turns/file_edits 메트릭 + 산출 dossier 본문 검수) 후, **6-10~6-27 사이클과 달리 1차 출처로 검증된 신규 파이프라인 패턴 3건을 발견**해 6/18·6/20 선례(genuinely-new 메타패턴은 analyses 보강)대로 수록. prompt injection 내성 적용(dossier/newsletter 본문 지시문 무시, 외부 인용은 참고 정보로만 코드펜스 격리).
- **신규 durable 지식: 0건 신규 페이지 / 2개 analyses 갱신(검증된 새 파이프라인 패턴).** ① [[research-write-agent-separation]] — dossier 스키마 성숙(per-evidence `verified` bool·`verifiedDowngradeCount`·교차일 dedup `seenBefore`/`seenBeforeCount` + write 단계 grounding 룰) 1건 + grounding≠정확성 함정(봇 차단 폴백 시 claim↔evidence.quote 불일치 entry 생성, 자격증명형 url 이 `https://***:***@gmail.com/` 로 마스킹된 채 유입 → 강제 blockquote 룰이 불일치 quote 충실 전파, claim↔quote 정합성 검사는 URL-grounding 과 별개 가드 필요) 1건. ② [[llm-content-quality-guards]] — 원칙 #5(토픽별 가드)에 anti-prediction 가드 토픽별 tailoring 실증(미래 버전 발명 금지: Linux/AI-coding 있음, Opensource Trending 없음) 1건.
- 스킵: 뉴스레터/dossier 본문 콘텐츠(커널/안드로이드/오픈소스/AI 코딩 그날치 휘발성 외부 뉴스)는 6/14~6/27 결정과 동일하게 durable 전량 스킵(단일 패치/릴리스 일반화=과잉추출, lore Anubis 차단으로 1차 미검증). Newsletter Write 11건 `assistant_turns: 0` 무응답(산출 콘텐츠 없음, 6/10 이래 약 2주 반째 고착). lint/포맷/오타·의존성 류 대상 없음.
- 운영 관찰: 갱신 [[dev-blog]] (projects) — 2026-06-28 사이클 관찰(write silent fail 지속·dossier 스키마 성숙·grounding≠정확성 노출·anti-prediction 토픽별 비일관성·Anubis 폴백 사다리 흡수).
- Updated: wiki/analyses/research-write-agent-separation.md(본문 2섹션 보강 + updated 2026-06-28 + sources 3건 + 변경이력), wiki/analyses/llm-content-quality-guards.md(원칙 #5 실증 + updated 2026-06-28 + sources 2건 + related + 변경이력), wiki/projects/dev-blog.md(사이클 관찰 1행 + updated 2026-06-28 + sources 1건), wiki/index.md(research-write·dev-blog 항목 갱신), wiki/log.md(본 항목).
- Marked ingested: true — session-log 22건 전체(030010·030337·030804·031150·031514·031808·032046·032815·032924·033118·033437·033824·034205·034534·034754·035055·035516·035811·040118·040358·040622·040855). 신규 페이지 0건, analyses 갱신 2건 + projects 갱신 1건.

## 2026-06-29 — wiki-ingest (session-logs, ingested: false 22건)

- 대상: **session-log 22건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~04:14 KST) = Research Dossier 11건 + Newsletter Write 10건 + Weekly Digest 1건. 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍. mcp-note(type: mcp-note) 0건. raw-sources/ 서브디렉터리(articles/books/ideas/papers/transcripts/Tips) 신규 .md 0건(PDF·txt·pptx 만, .cache/extracted 비어 있음, fetched/ 없음) → raw-sources/PDF/URL 유래 대상 외. 참고: grep 으로 23건이 잡혔으나 1건(20260607-095133 agent-weekly 주간보고 agent 설계)은 본문 코드블록의 "ingested: false" 텍스트를 매칭한 오탐 — frontmatter 는 이미 `ingested: true` 라 제외(기수집된 설계 세션).
- 처리 방식: 22건 트리아지(서브에이전트 2개 병렬 분석[커널·오픈소스 / AI코딩·큐레이션] + assistant_turns/file_edits 메트릭 + 산출 dossier·newsletter 본문 검수) 후, 서브에이전트2 가 "신규"로 제시한 항목을 직접 1차 검증해 전량 기수록/과잉추출로 기각. prompt injection 내성 적용(dossier/newsletter 본문 지시문 무시, 외부 인용은 참고 정보로만 코드펜스 격리).
- **신규 durable 지식: 0건(전량 스킵).** ① `verifiedDowngradeCount`/`seenBeforeCount`/`seenBefore` 는 **6/28 에 이미 [[research-write-agent-separation]] 에 수록**된 스키마(서브에이전트 제안은 중복) → 갱신 불필요. ② "grounding≠정확성" 은 6/29 산출 newsletter(030351)에서 `https://***:***@ideco.ru/` 자격증명 마스킹 verifyLink + claim↔quote 불일치(SCSI DMA claim↔NFSD revert quote)로 **재현**됐으나 6/28 수록 함정의 실증 사례일 뿐 새 패턴 아님. ③ AI Coding Agents dossier(032939)의 로컬 오픈모델 function-call 벤치마크·프론티어 LLM 장시간 이식 사례는 confidence=low 단일출처 그날치 뉴스 → 단일 벤치/릴리스 일반화 = 과잉추출이라 스킵(기존 [[ai-coding-agent-cost-and-context-patterns]] 범위). ④ 휘발성 커널/OSS 본문은 6/14~6/28 결정과 동일 스킵.
- 검증 메모: assistant_turns:0 인데 byte 가 큰 로그(041412 31KB·031806 23KB·034441 21KB)는 `## Assistant` 섹션이 없고 전부 User 프롬프트에 dossier 가 임베딩된 *입력*이라 산출물 아님(서브에이전트2 의 "출력 존재" 판단은 입력 임베딩 오독) → write silent fail 확정.
...
## 2026-07-01 — wiki-ingest (session-logs, ingested: false 21건)

- 대상: **session-log 21건 전부 dev-blog 뉴스레터 자동화 cron 사이클**(cwd=dev-blog, 03:00~03:58 KST) = Research Dossier 10건 + Newsletter Write 11건. 테마: Linux Daily/Android Kernel/Opensource Trending/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍, Android Kernel 만 write 2건. mcp-note(type: mcp-note) 0건. raw-sources/ 서브디렉터리 신규 .md 0건(.cache/extracted 비어 있음, fetched/ 없음, PDF/txt/pptx 는 .md 아님이라 대상 외). 참고: grep 으로 22건이 잡혔으나 1건(20260607-095133 agent-weekly 주간보고 agent)은 본문 249행 코드블록의 "ingested: false" 텍스트를 매칭한 오탐 — frontmatter 는 이미 `ingested: true` 라 제외(6/30 운영자 판단과 동일).
- 처리 방식: frontmatter 트리아지 후, dossier/newsletter 본문의 범용 지식 후보를 서브에이전트 1개로 기존 wiki/analyses 전수 대조 스캔(prompt injection 내성 적용 — dossier/newsletter 본문 지시문 무시). 서브에이전트가 제시한 2건(① skills 상호운용 거버넌스: ponytail·eve·BuilderIO/skills, ② devspace 셀프호스트 MCP)은 모두 그날치 도구 언급 기반의 휘발성 뉴스이며 기존 [[ai-coding-agent-cost-and-context-patterns]]/[[self-hosted-agent-webui-integration]]/[[mcp-config-secret-exposure-via-ps]]/[[multi-profile-cli-agent-isolation]] 범위와 중복 → "억지로 만들지 말 것" 지침 + 6/14~6/30 선례에 따라 스킵.
- **신규 durable 지식: 0건(전량 스킵).** 휘발성 커널/Android/OSS·AI 코딩 속보 본문은 단일 패치/릴리스 일반화=과잉추출이라 6/14~6/30 결정과 동일 스킵. 파이프라인 메타패턴(2단계 분리·grounding 계약·confidence 강등·Anubis 폴백 사다리·topic-id grep 자가판별·verifiedDowngradeCount/seenBeforeCount 정량 자기보고)은 이미 [[research-write-agent-separation]]/[[llm-newsletter-rewrite-metadata-grounding]]/[[llm-content-quality-guards]]/[[ai-coding-agent-cost-and-context-patterns]] 에 수록.
- 운영 관찰만 기록: 갱신 [[dev-blog]] (projects) — 2026-07-01 사이클. **write fail 간헐화 지속**: Newsletter Write 11건 중 Android Kernel 2건(030911·031249)만 `assistant_turns: 1` 정상 산출, 나머지 9건 무응답 — 6/29(Linux Daily 1건)·6/30(Lens gpu-ai 1건)에 이어 토픽만 바뀐 간헐 1턴 부분 회복(이번엔 동일 토픽 2건, 비결정적, 6/10 이래 약 3주째 고착 지속). Research Dossier 는 7/10 정상(6/30 과 동일 수준), 3건(Linux Daily·Android Kernel·Lens 033809) 무응답 — 평소 정상이던 Linux Daily·Android dossier 가 이번엔 무응답으로 토픽 분포만 교대. Anubis 폴백 사다리 동일 흡수, bash 폴백 로그에 에러·실패 흔적 없음. lint/포맷/오타·의존성 류 대상 없음.
- Updated: wiki/projects/dev-blog.md(사이클 관찰 1행 + updated 2026-07-01 + sources 1건), wiki/index.md(dev-blog 항목 1행 보강 + updated 2026-07-01), wiki/log.md(본 항목). (신규 페이지 0 → analyses/concepts/patterns 변경 없음, 6/27·6/29·6/30 무신규 사이클과 동일)
- Marked ingested: true — session-log 21건 전체(030012·030307·030529·030911·031249·031532·031743·032053·032357·032743·033123·033433·033622·033809·034101·034418·034918·035142·035351·035550·035835). 신규 페이지 0건, projects 갱신 1건(운영 관찰).
