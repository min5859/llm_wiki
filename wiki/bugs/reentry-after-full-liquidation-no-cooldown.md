...
## 후속 정정 — 60분은 여전히 비대칭이었다 (2026-06-11)

최초 수정의 `reentry_cooldown_minutes=60` 은 가드 공백을 메웠지만 **값이 비대칭**이라는 사실이 뒤늦게 드러났다. 분할 추가매수 throttle(`min_split_interval_minutes`)은 같은날 재매수를 **다음날(1080분=18시간)** 까지 막는데, 전량 청산 후 재진입만 60분으로 풀어둔 것. 결과적으로 *전량 트레일링 청산 → 60분 후 반등에 재매수 → 다시 손절* 의 **같은날 휩쏘**가 여전히 가능했다.

두 가드(분할 throttle / flat 재진입 쿨다운)는 **같은 위험(같은 종목 재매수)** 을 덮으므로 타이밍이 일치해야 한다. `reentry_cooldown_minutes: 60 → 1080` 으로 정렬해 전량 청산된 종목은 다음날 첫 매수(09:30 시각 가드와 맞물림)까지 재진입 불가 (commit `c4cc530`, 기존 6 테스트 통과).

> 추가 교훈: **같은 위험을 덮는 두 가드는 타이밍을 일치시켜라.** 한쪽(분할)은 다음날, 다른쪽(flat 재진입)은 60분이면 "쿨다운이 있다"는 착각 속에 같은날 재진입이 새어나간다.
...
- 2026-06-11: 「후속 정정」 절 추가 — `reentry_cooldown_minutes` 60→1080 으로 정렬, 분할 throttle 과의 비대칭 해소 (commit `c4cc530`, 출처: session-logs/20260611-231312-ba46-*)
