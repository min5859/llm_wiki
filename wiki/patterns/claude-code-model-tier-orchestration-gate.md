...
**Bash 인플레이스 감지는 정규식이 아니라 shlex 토큰화로** (7/14 재작성). `\bsed\b.*\s-i` 류 정규식 스캔은
오탐 4종이 실증됐다: 읽기 전용 `sed -n … | grep -i`(파이프 뒤 `-i`를 sed 것으로 오인), 커밋 메시지 안
"sed -i" 문자열, `perl -ne … | grep -in`, `echo "a > b.py"`(따옴표 안 `>`). 근본 해결은 **따옴표·heredoc
본문을 데이터로 취급하는 shlex 기반 토큰화 + 파이프라인 세그먼트 단위 판정 + `bash -c '…'` 내부 재귀 스캔**.
덤으로 `sed -ri` 클러스터 플래그·`gsed`·인용된 리다이렉트 대상(`> "my file.py"`)까지 커버가 넓어진다.
FP 7종·TP 9종·회귀 포함 23/23 검증.

> **핵심 재사용 사실 — Claude Code 훅에서 "모델에 조언만 주입"(non-blocking nudge)**:
...
> ```json
> {"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"…조언…"}}
> ```
> ⚠️ **`permissionDecision:"allow"` 를 함께 넣으면 안 된다** — 해당 툴콜이 **권한 프롬프트 없이 자동 승인**된다.
> 즉 넛지가 발동하는 바로 그 순간(의심스러운 `sed -i` 등)에 사용자 승인을 우회하는 부작용 (7/14 실증 후 제거.
> 공식 문서 확인: `additionalContext` 단독 반환이면 조언은 주입되고 권한 흐름은 평소대로 진행된다).
...
- **hard 모드는 CC v2.1.196+ 필수**: 구버전은 payload 에 `agent_id` 가 없어 게이트가 implementer 등
  서브에이전트의 편집까지 차단한다 — "정밀도만 낮아짐"이 아니라 **위임 자체가 깨진다**. soft 는 넛지뿐이라
  구버전에서도 영향 미미(턴 카운팅만 transcript fallback 으로 정밀도 저하).
- **모드 지침의 서브에이전트 누출**: `active.md` 는 CLAUDE.md import 라 **서브에이전트 컨텍스트에도 로드**된다.
  implementer 가 "당신은 오케스트레이터(Opus)다"를 함께 읽는 정체성 혼동 — mode-hard/soft.md 상단에
  "메인 세션 전용, 서브에이전트는 자기 지침 우선" 한 줄로 방어 (implementer.md 쪽 게이트 예외 문구만으론 부족).
- **모델 핀 드리프트**: env.sh 의 모델 하드코딩(`claude-opus-4-8` 등)은 세대 교체(Claude 5 계열) 때 조용히
  어긋난다. `/model`·`--model` 이 env 보다 우선함이 실증됨(hard 모드인데 메인이 Fable 5 로 동작). 세대 교체
  시 핀을 수동 재점검하는 운영 항목으로 관리.
- **state 파일 누적**: `state/<session_id>.json` 이 세션마다 생기고 정리 로직이 없으면 무한 증가 →
  save_state 시점에 오래된 파일(예: 7일 초과) prune.
...
- **settings.json 파싱 실패 시 진행하지 말고 중단**: 실패를 `cfg = {}` 로 삼키고 병합을 계속하면 기존
  설정 전체가 훅만 남은 파일로 덮인다(백업 1회가 있어도 위험). 파싱 실패 = abort 가 정답.
...
- 2026-07-14: 실전 검토(출처: session-log dfcb)로 실증 결함 2건 정정 — (1) 소프트 넛지의 `permissionDecision:"allow"` 가 권한 프롬프트를 자동 승인(우회)하는 부작용 실증 → `additionalContext` 단독 반환으로 정정 (핵심 재사용 사실 블록의 JSON 수정), (2) Bash 인플레이스 정규식 감지의 오탐 4종 실증(파이프 뒤 `grep -i`·커밋 메시지 문자열·`echo "a > b.py"` 등) → shlex 따옴표·heredoc 인지 토큰화 + 파이프라인 세그먼트 판정 + `bash -c` 재귀 스캔으로 재작성(23/23 검증). 함정 4건 보강: hard 모드 CC v2.1.196+ 필수(구버전 `agent_id` 부재로 위임 자체 붕괴), 모드 지침 서브에이전트 누출("메인 세션 전용" 헤더로 방어), env.sh 모델 핀 드리프트(`/model` 우선 실증), state 파일 prune. 배포 절에 install.sh 파싱 실패 시 중단 원칙 추가.
