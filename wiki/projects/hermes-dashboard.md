...
- 2026-07-05: 재부팅 후 복구 세션 정리 — webui/게이트웨이 자동 기동 등록([[hermes]] 운영 회고), 위임 이벤트 실측 형태·`API_SERVER_KEY` 세션 연속·`messages_fts` 검색은 [[hermes-agent]] 로, 6/21 선구현했던 위임 파서가 라이브 캡처로 최종 검증됨(스펙 문서·코드 어휘 모두 실물과 달랐음 — 파서 선구현 + 실캡처 검증 패턴 재확인). 활성 게이트웨이 전환의 live-binding + candidate allowlist(SSRF 가드) 동작 확인. 후반부 Hermes 멀티에이전트 운영 실험(공유 llm-wiki·칸반 파이프라인·Paperclip PoC)은 [[multi-agent-shared-wiki-concurrency]]·[[single-dispatcher-per-queue]]·[[multi-agent-orchestration-taxonomy]]·[[weak-model-agent-reliability-compounding]] 로 분리 (출처: session-logs/20260704-132738-e509-*)
