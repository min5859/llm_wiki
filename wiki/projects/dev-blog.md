...
sources:
  ...
  - "session-logs/20260729-045103-cb54-#-Linux-Kernel-Lens-Newsletter-—-Write-from-Dossie.md"
  - "session-logs/20260729-050037-8470-#-Linux-Kernel-Lens-Newsletter-—-Write-from-Dossie.md"
  ...
  - "session-logs/20260728-040143-bbb5-#-Linux-Kernel-Lens-Newsletter-—-Write-from-Dossie.md"
  - "session-logs/20260728-030016-36fe-#-Linux-Daily-Research-Dossier-당신은-리눅스-커널-개발-뉴스레터의.md"
  ...
  - "session-logs/20260704-030017-0e55-#-Linux-Daily-Research-Dossier-당신은-리눅스-커널-개발-뉴스레터의.md"
  - "session-logs/20260701-032053-3c30-#-AI-Coding-Agents-Research-Dossier-당신은-AI-코딩-에이전트.md"
  - "session-logs/20260630-030015-b621-#-Linux-Daily-Research-Dossier-당신은-리눅스-커널-개발-뉴스레터의.md"
  - "session-logs/20260629-030010-3a14-#-Linux-Daily-Research-Dossier-당신은-리눅스-커널-개발-뉴스레터의.md"
  ...
  - "session-logs/20260727-044622-1335-#-Linux-Kernel-Weekly-Digest-당신은-리눅스-커널-개발-주간-다이제스.md"
confidence: high
updated: 2026-07-29
related:
...
- 2026-06-28 (03:00 cron 사이클 22건 — write silent fail 약 2주 반째 + dossier 스키마 성숙 관측): research(dossier)→write(newsletter) 2단계 cron 이 03:00~04:08 KST 에 발사 (Research Dossier 11건 + Newsletter Write 11건; 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍, Android Kernel·Linux Kernel Lens 일부 write 2건). **운영 관찰**: (1) **Newsletter Write 단계가 여전히 `assistant_turns: 0` 무응답** — 6/24~6/27 와 동일, 6/10 이래 약 2주 반째 비결정적 고착 지속. (2) dossier 입력 스키마가 성숙 — per-evidence `verified`(bool)·dossier 레벨 `verifiedDowngradeCount`·교차일 dedup `seenBefore`/`seenBeforeCount` 필드가 확인됨(write 프롬프트에도 `seenBefore` 대응 룰 추가). 기존 confidence 강등이 산문이던 것이 스키마 수준 정량 자기보고로 진화 → 일반 패턴은 [[research-write-agent-separation]] 에 추가. (3) **grounding≠정확성 노출** — Linux Daily dossier(030010) 의 "Linux 7.1 릴리스" entry 가 claim↔evidence.quote 불일치(RollBall/RCU quote)에 일부 `verified:false`, url 은 자격증명형 `user:pass@` 가 마스킹돼 `https://***:***@gmail.com/` 로 유입. 봇 차단 폴백이 후보 payload 인용을 그러모을 때 상류 조사 오류가 grounding 가드를 통과해 전파되는 사례 → 함정으로 [[research-write-agent-separation]] 에 추가. (4) anti-prediction 가드의 토픽별 비일관성(미래 버전 발명 금지: Linux/AI-coding 있음, Opensource Trending 없음) → [[llm-content-quality-guards]] 원칙 #5 실증으로 추가. (5) Anubis 봇 차단(`lore.kernel.org`) 폴백 사다리는 6/18~6/27 와 동일하게 흡수, bash 폴백 로그에 에러·실패 흔적 없음. 휘발성 커널/OSS·CC 릴리스 속보 콘텐츠 자체는 6/14~6/27 결정과 동일하게 durable 전량 스킵. **코드 변경 없음** (출처: session-logs/20260628-03* 03:00~04:08 사이클 22건)
- 2026-06-29 (03:00 cron 사이클 22건 — write silent fail 약 2주 반째 + **간헐 1턴 회복 1건 관측**): research(dossier)→write(newsletter) 2단계 cron 이 03:00~04:14 KST 에 발사 (Research Dossier 11건 + Newsletter Write 10건 + Weekly Digest 1건; 테마: Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍). **운영 관찰**: (1) **Newsletter Write 10건 중 9건이 `assistant_turns: 0` 무응답**이나 **Linux Daily Write(030351)만 `assistant_turns: 1` 로 정상 산출** — 6/24~6/28 의 *완전* 무응답에서 6/22 처럼 *간헐 1턴 부분 회복*으로 변화(원인 미상·비결정적). 6/10 이래 약 2주 반째 write 단계 비결정적 고착은 지속. Weekly Digest(041412)는 `assistant_turns: 0` 무응답. (2) Research Dossier 는 6건만 1턴 정상(030010 Linux Daily·030740 Android·032939 AI Coding·035023·035630·040747 Lens), 5건은 `assistant_turns: 0` 무응답(031446 Opensource Trending·032549 Opensource Curation·033826·034441·040150 Lens) — 6/27(10/11)·6/28 대비 **dossier 측 산출이 6/11 로 퇴행**, research/write 비대칭 지속. (3) **grounding≠정확성 재확인(신규 아님)** — 정상 산출된 Linux Daily newsletter(030351)의 `verifyLink`·`[원문 확인]` URL 이 자격증명형 `user:pass@` 가 마스킹된 `https://***:***@ideco.ru/` 로 들어오고 highlight 의 claim↔quote 가 불일치(SCSI DMA 패딩 미초기화 claim 아래 NFSD umount revert quote) — 6/28 에 [[research-write-agent-separation]] 에 수록한 함정의 재현 사례(새 패턴 아님, write 단계가 `verified:false` quote 를 여전히 억제 못 함을 실증). (4) AI Coding Agents dossier(032939)는 1턴 산출했으나 휘발성 외부 뉴스(로컬 오픈모델 function-call 정확도 벤치마크·프론티어 LLM 장시간 이식 사례 등 confidence=low 단일출처) — 단일 릴리스/벤치를 범용 패턴으로 일반화 = 과잉추출이라 스킵, 기존 [[ai-coding-agent-cost-and-context-patterns]]/[[claude-code-source-leak-internals]] 범위. (5) Anubis 봇 차단(`lore.kernel.org`) 폴백 사다리는 6/18~6/28 와 동일하게 흡수, bash 폴백 로그에 에러·실패 흔적 없음. 휘발성 커널/OSS·CC 속보는 6/14~6/28 결정과 동일하게 durable 전량 스킵. 파이프라인 메타패턴(verifiedDowngradeCount·seenBeforeCount 정량 자기보고 포함)은 이미 [[research-write-agent-separation]]/[[llm-newsletter-rewrite-metadata-grounding]]/[[llm-content-quality-guards]]/[[ai-coding-agent-cost-and-context-patterns]] 에 수록. **코드 변경 없음** (출처: session-logs/20260629-03* 03:00~04:14 사이클 22건)
- 2026-06-30 (03:00 cron 사이클 20건 — write silent fail 약 3주째 + 간헐 1턴 회복 1건 지속): research(dossier)→write(newsletter) 2단계 cron 이 03:00~03:58 KST 에 발사 (Research Dossier 10건 + Newsletter Write 10건; 테마: Linux Daily/Android Kernel/Opensource Trending/AI Coding Agents 각 1쌍 + Linux Kernel Lens 6쌍 [linux-kernel-security·linux-toolchain·linux-distro-stable·linux-perf-rt·linux-arch-platform·linux-gpu-ai]). **운영 관찰**: (1) **Newsletter Write 10건 중 9건이 `assistant_turns: 0` 무응답**이나 **Linux Kernel Lens(gpu-ai, 035825) Write 만 `assistant_turns: 1` 로 정상 산출** — 6/29(Linux Daily 1건)에 이어 *간헐 1턴 부분 회복*이 토픽만 바뀐 채 지속(원인 미상·비결정적). 6/10 이래 약 3주째 write 단계 비결정적 고착. (2) Research Dossier 는 7건 1턴 정상(Linux Daily·Android·AI Coding + Lens toolchain·perf-rt·arch-platform·gpu-ai), 3건은 `assistant_turns: 0` 무응답(Opensource Trending·Lens security·Lens distro-stable) — 6/29(6/11) 대비 dossier 측 회복(7/10), research/write 비대칭 지속. (3) **topic-id grep 자가판별 재확인(신규 아님)** — linux-toolchain dossier(033353)에서 프롬프트에 topic 이 주입되지 않자 에이전트가 `grep -rl "<sourceId>" content/topics/*/sources.json`(sourceId=`lore-rust-for-linux-new`·`lore-llvm-new`·`lore-linux-kbuild-new`)으로 실제 렌즈 매핑을 역추적한 뒤 droppedCandidates 판단 — 6/20 에 [[research-write-agent-separation]] 에 수록한 self-grounding 가드레일의 재현. (4) Anubis 봇 차단(`lore.kernel.org`) 폴백 사다리(raw mbox·commitMessage+WebSearch 교차검증·미확인 confidence 강등+openQuestions 격리)는 6/18~6/29 와 동일하게 흡수, bash 폴백 로그에 에러·실패 흔적 없음. 휘발성 커널/OSS·AI 코딩 속보 콘텐츠(ioc3-eth UAF·UFS fix chain·memblock 불필요 범위검사·drm/omap DSI BTA·amdgpu bo_va 핸들 race·strscpy 치환 시리즈 등)는 **단일 패치를 범용 패턴으로 일반화 = 과잉추출**이라 6/14~6/29 결정과 동일하게 durable 전량 스킵. AI 코딩 dossier 의 skills 포맷 벤더 수렴·meta-harness·Codex CLI 도 그날치 뉴스로 이미 [[ai-coding-agent-cost-and-context-patterns]]/[[openai-codex-cli-overview]]/[[everything-claude-code]] 범위. **코드 변경 없음** (출처: session-logs/20260630-03* 03:00~03:58 사이클 20건)
- 2026-07-03/04 (03:00 cron 사이클 13+11건 — write 로그 소실 확대 + 후보 공급 정체 + 신규 함정 3건): research(dossier)→write 2단계 cron 발사 (7/3: dossier+write 13건, 7/4: 11건 — **7/4 는 write 세션 로그 자체가 0건**으로 write 측 관측이 로그 미캡처로 확대, 7/3 write 도 산출 본문 미기록). **운영 관찰**: (1) **후보 공급 정체 신호** — Android Kernel 후보 11건이 7/3·7/4 완전 동일 세트 재투입, 7/3 Android dossier 는 entry 10건 중 9건 `seenBefore: true` (delta 없는 날의 뉴스레터가 사실상 재탕이 되는 케이스). (2) **evidence-stuffing 관측** — 7/3 Linux Daily dossier(030007)에서 후보 JSON 손상+봇 차단 하에 무관 quote/URL 이 evidence 로 채워지고 confidence 강등 불변식까지 위반(`confidence: high` 유지) → 신규 함정으로 [[research-write-agent-separation]] 에 추가. (3) **마스킹의 span 파괴** — 양일 lens·daily 프롬프트의 후보 JSON 이 `https://`~`@host` 구간 단위로 `***:***` 치환돼 스키마 예시와 병합·붕괴(message-ID `@domain` 오탐 추정, 파이프라인 송신분 vs 세션로그 스크러버 원인 미확정 — 소스 확인 필요) → 상동. (4) **수집기 MIME 미디코딩** — 7/4 lens(054c) 후보 `commitMessage` 2건이 base64 원문 그대로 유입(quoted-printable 잔재 `=20` 등도 산재) — grounding 재료 소실+토큰 낭비, collect 단계 디코딩 필요(미구현, 제안만 기록). (5) 7/3 Android write 프롬프트에서 quote 검증 실패 → `openQuestions` 자동 주입(`verifiedDowngradeCount: 4`) 확인 → [[research-write-agent-separation]] 에 추가. 뉴스 콘텐츠는 기존 결정과 동일하게 durable 전량 스킵. **코드 변경 없음** (출처: session-logs/20260703-03*, 20260704-03* 사이클 24건)
...
- 2026-07-27 (03:00~04:49 cron 사이클 21건 — write 정상 지속 + lore 차단 5번째 관측 + Weekly Digest 입력 결손 전파 실측): Research Dossier 11건 + Newsletter Write 9건 + Weekly Digest 1건. research 는 bash 12~35회 실측 조사, write 는 도구 없는 순수 산출 패턴으로 정상(에이전트형 표류 재발 없음). lore.kernel.org 차단 5번째 관측 — `git/2.43` UA 가 nginx 레벨 403 필터·Anubis 챌린지 양층을 모두 통과해 raw mbox 정상 수신(상세 [[newsletter-research-anti-bot-blocking]]). Linux Kernel Weekly Digest(044622) 입력 wrapper 의 `dailies` 가 프롬프트 계약("지난 7일치") 대비 **3건**(`draftMetadata.rewrittenAt` 기준 07-19·07-23·07-25)만 포함 — 주중 daily 결손이 주간 다이제스트 입력 결손으로 전파됨을 실측. weekly wrapper 에는 `externalSignals`(lwn.net·phoronix RSS 항목)·`mainlineMerges` 필드가 파이프라인에 의해 주입됨 — 07-06 digest 입력엔 없고 07-13 부터 존재(그 사이 파이프라인 확장으로 추정). **코드 변경 없음** (출처: session-logs/20260727-041837-175a-*, -044622-1335-* 외 20260727 cron 21건)
- 2026-07-27/28 (07-27 21:31~21:46 저녁 이탈 실행 4건 + 07-28 03:00~05:01 새벽 cron 22건 — 저녁 시간대 실행 실측 + verified 플래그 자체 오염 + credential 마스킹 파괴 지속): 07-27 21:31~21:46 에 linux-distro-stable·linux-toolchain 의 research/write 쌍(094c·3e22 → d0f3·432f) 이 실행됨 — 평소 새벽 03~05시 cron 사이클 밖 시간대 실행이 이번에 처음 실측됨. 원인은 로그상 미상 — 단정하지 않고 [[macos-launchagent-catchup-behavior]] 가능성만 언급. **claim↔quote 정합성 갭 재발, 한 단계 악화**: 07-28 새벽 bbb5(Linux Kernel Lens write) dossier 에서 "audit_log_n_hex() 시프트 오버플로우 수정" claim 에 전혀 무관한 fsl-mc strscpy 패치의 b4-ty quote("Applied, thanks! [1/1] drivers/bus/fsl-mc: Use strscpy() to copy strings into arrays …")가 **`verified: true` 상태로** 부착됨 — 06-28 기록한 "grounding≠정확성" 갭보다 한 단계 나쁜, 검증 플래그 자체가 오염된 사례(같은 dossier 의 `verifiedDowngradeCount: 0` — 강등조차 없었음). [[research-write-agent-separation]] 링크. **credential 마스킹 span 파괴 지속**: bbb5 의 evidence url 이 전면 `https://***:***@…` 형태로 손상됐고, 36fe(Linux Daily dossier) 의 patchwork msgid 조회 URL 도 `https://***:***@gmail.com/raw` 로 파괴(message-id 의 `@domain` 오탐) — [[gieok-session-log-url-credential-masking-false-positive]] 기수록 패턴의 재발. `seenBeforeCount` 고수준 지속(Android Kernel dossier 9건 등, 재순환 특성 기존 판정 유지). lore.kernel.org 차단 6번째 관측은 [[newsletter-research-anti-bot-blocking]] 로 분리. **코드 변경 없음** (출처: session-logs/20260727-213100-094c-*, -213102-3e22-*, -214304-d0f3-*, -214611-432f-*, 20260728-040143-bbb5-*, -030016-36fe-* 외 07-27 저녁·07-28 새벽 cron 26건)
- 2026-07-29 (03:00~05:00 새벽 cron 21건 — 교정 재시도 cron 실사용 첫 실측 + Kernel Lens 로그 소실 4/6 확산 + anti-bot 7번째 관측): Research Dossier 6(Kernel Lens 6렌즈) + Linux Daily/Android Kernel/Opensource Trending/Opensource Curation/AI Coding Agents 의 R+W 10 + Kernel Lens Write 4. **교정 재시도 실사용 첫 실측** — Opensource Trending write(`eff4`, 03:29)가 1차 시도 후, 03:31(`cec7`) 동일 프롬프트 끝에 `[재시도] 직전 응답이 유효한 JSON이 아니었습니다. 파일 읽기/쓰기나 도구 사용 없이, 설명 문장 없이, 유효한 JSON 객체 하나만 출력하세요.` 교정 지시문이 붙어 재발사됨 — 2026-07-22 commit `e28df77` 로 도입된 attempt≥2 교정 재시도 경로가 cron 실운영에서 발사된 것을 세션 로그로 처음 확인. `cec7` 도 `assistant_turns: 0` 으로 종료돼 최종 성공 여부는 로그에 남지 않음(헤드리스 로거 특성). 상세는 [[llm-json-parse-retry-with-dump]]. **Kernel Lens 로그 소실이 4/6 렌즈로 확산** — research 된 6렌즈 중 `b662`(보안)·`d7dd`(stable)·`3f8a`(arch/전력관리)·`d39c`(GPU) 4개는 세션 로그에 최종 dossier JSON 이 남지 않음(Assistant 텍스트도 `Write` 파일 콜도 없이 bash 탐색만 기록). 이 중 stable·arch-platform·gpu-ai 3개는 대응 write(`082f`·`cb54`·`8470`)가 dossier 를 입력으로 받아 정상 발사돼 산출물 자체는 실제로 생성됐음이 확인됨 — 로거의 assistant 출력 소실 특성(07-20 이래 기수록)의 연장. 보안 렌즈(`b662`)는 이번 배치에 대응 write 가 아예 없어 산출 여부가 미확정이며, perf-rt(`425d`)는 research 자체는 Assistant 턴에 dossier 전문을 출력해 완료했으나 역시 대응 write 가 배치에 없음(부재가 결손인지 로그 미캡처인지는 미확정). `feda`(툴체인)만 `/tmp/lore/dossier.json` 에 명시적 `Write` + 파이썬 스키마 검증(`valid True`, entries=6)까지 로그에 남았다. **anti-bot 7번째 관측** — lore.kernel.org 차단 6일 연속 지속, 브라우저 Mozilla UA 가 이번엔 `200`+Anubis 챌린지 대신 `500 Server Error`("Oh noes!")를 반환하는 신규 변형 관측(`3f8a`) + NNTP 직접 접근의 GREET/ARTICLE/HEAD/BODY 프로토콜 상세가 처음 확정(`1ece`) — 상세는 [[newsletter-research-anti-bot-blocking]] 7번째 관측. **credential 마스킹 span 파괴 지속** — lens write 4건(`c176`·`082f`·`cb54`·`8470`) 전부 evidence url 이 `https://***:***@…` 형태로 파괴됨 — [[gieok-session-log-url-credential-masking-false-positive]] 기수록 패턴 재발. 뉴스레터 기사 콘텐츠는 기존 결정대로 durable 전량 스킵. **코드 변경 없음** (출처: session-logs/20260729-033152-cec7-*, -035106-b662-*, -040000-feda-*, -041304-d7dd-*, -042651-425d-*, -044041-3f8a-*, -045245-d39c-*, -030012-1ece-* 외 20260729 cron 21건)
- 2026-07-30 (03:00~05:00 새벽 cron 23건 — 교정 재시도 2번째 실측[이틀 연속 동일 토픽] + Kernel Lens research 로그 소실 6/6 전면화): Research Dossier 11건(Linux Daily·Android Kernel·Opensource Trending·Opensource Curation·AI Coding Agents + Kernel Lens 6렌즈) + Newsletter Write 12건(Trending 더블런 포함). **운영 관찰**: (1) **교정 재시도 2번째 실측** — Opensource Trending write(`cfd8`, 03:32) 1차 후 03:35(`d0f7`) 동일 dossier 에 `[재시도] 직전 응답이 유효한 JSON이 아니었습니다 …` 교정 지시문이 붙어 재발사. 07-29(`eff4`→`cec7`)에 이어 이틀 연속이며 두 번 모두 Opensource Trending write 에서만 발화 — 실패가 무작위가 아니라 토픽(입력 dossier 형태)에 상관될 가능성 시사(표본 2회, 미확정). `d0f7` 도 `assistant_turns: 0` 종료로 최종 성공 여부는 로그 미기록. 상세는 [[llm-json-parse-retry-with-dump]]. (2) **Kernel Lens research 로그 소실 6/6 전면화** — 07-29 의 4/6 에서 확대: 6렌즈 research(`99aa`·`c795`·`1d1c`·`15e7`·`0642`·`a43c`) 전건이 bash 탐색(렌즈별 0~19회)만 기록되고 Assistant 턴·최종 dossier JSON 이 로그에 없음. 단 대응 Lens write 6건(`d96f` security·`8972` toolchain·`bdd6` distro-stable·`5eac` perf-rt·`93a7` arch-platform·`fdfd` gpu-ai)이 모두 dossier 를 입력으로 수신해 발사됐으므로 dossier 산출 자체는 이뤄짐 — 로거의 assistant 출력 소실 특성(07-20 이래 기수록)의 연장. daily 계열도 동일(Android research `decc` 는 1턴 계획 선언만 기록됐으나 대응 write `d8f4` 가 완성 dossier 수신). (3) write 12건 중 11건 `assistant_turns: 0`, distro-stable `bdd6` 만 1턴인데 "도서관 dossier를 기반으로 뉴스레터 JSON을 작성합니다." 한 줄 선언 후 산출 없이 종료 — 산출 여부는 로그만으로 미확정(07-25 이래 "정상 write 는 로그에 안 남는" 관측 프레임 유지). (4) **anti-bot 8번째 관측, 신규 변형 없음** — gpu-ai research(`a43c`)의 UA 스윕 실측이 07-28 판정과 일치: `Wget/1.21.4`·`git/2.44.0`·`public-inbox-fetch` → `200` raw mbox, `python-requests/2.31.0`·빈 UA → `403`, 브라우저(Googlebot Mozilla) UA → `200`+Anubis 챌린지 HTML("Making sure you're not a bot!"). 자기서술적 비브라우저 UA 통과(07-28 `public-inbox-mirror`)·git 버전 무관 판정의 재확인이라 [[newsletter-research-anti-bot-blocking]] 본문 변경 없음. credential 마스킹 span 파괴([[gieok-session-log-url-credential-masking-false-positive]])·`seenBefore` 재순환도 기수록 재발. 뉴스 콘텐츠(AI Coding: Cursor iPad 릴리스·Copilot 정책 전환 등 포함)는 기존 결정대로 durable 전량 스킵. **코드 변경 없음** (출처: session-logs/20260730-03*, 20260730-04*, -050027-* 사이클 23건)
- 2026-08-02 (03:00~04:36 새벽 cron 21건 — anti-bot 11번째 관측 판정 동일 + 로그 소실 패턴 지속): Research Dossier 11건(Linux Daily·Android Kernel·Opensource Trending·Opensource Curation·AI Coding Agents + Kernel Lens 6렌즈) + Newsletter Write 10건. **운영 관찰**: (1) **anti-bot 11번째 관측, 신규 변형 없음** — lore.kernel.org 차단 07-24 이래 10일 연속(`91e9`·`ccc6` Anubis 챌린지 실측). Kernel Lens research(`ccc6`, 03:52)의 UA 3종 스윕이 기존 판정과 일치: curl 기본 UA·`curl/8.7.1` → nginx bare `403`(146B), `git/2.39.0` → `200` raw mbox(4359B) — 기존 실측(2.43·2.44·2.45·2.53)보다 낮은 git 버전으로는 첫 통과 실측이라 "버전 무관" 판정을 하향 방향으로도 재확인. 브라우저 Mozilla UA 는 bare `Mozilla/5.0`(`91e9`, 03:41)·풀 Chrome UA(`ccc6`) 양쪽 모두 `200`+Anubis 1.25.0 챌린지(difficulty 4)로 07-31 복귀 형태 유지, kernel.org releases.json `200` 재확인 — 판정 동일이라 [[newsletter-research-anti-bot-blocking]] 본문 변경 없음(07-30 8번째 관측과 동일 처리). (2) **로그 소실 패턴 지속** — 비-Lens research 5건은 프롬프트만 기록(0턴), write 10건 중 9건 `assistant_turns: 0`, perf-rt write(`fc0f`)만 1턴에 완성 JSON 뉴스레터 출력 — 07-20 이래 기수록 "헤드리스 로거 산출 소실" 프레임 내 수준, 질적 신규성 없음. (3) **기수록 재발 2종** — AI Coding(`9594`)·distro-stable(`ed96`) dossier 의 `verifiedDowngradeCount` 2·1 은 06-28 [[research-write-agent-separation]] 기수록 quote 검증 강등의 재발, Lens write 로그 4건(`bdd0`·`30fc`·`ed96`·`fc0f`)의 evidence url `https://***:***@…` 마스킹 span 파괴는 [[gieok-session-log-url-credential-masking-false-positive]] 기수록 재발. 뉴스 콘텐츠는 기존 결정대로 전량 스킵. **코드 변경 없음** (출처: session-logs/20260802-034039-91e9-*, -035136-ccc6-*, -033800-9594-*, -041000-ed96-*, -042348-fc0f-* 외 20260802 cron 21건)
- 2026-08-02 (구조적 수정 — 반복 실패의 3부류 대응, commits `1608b20`·`239c5b7`): 사용자 "아직도 실패" 제보로 6일(07-28~08-02) 실패 census. **재구성**: 12토픽 fleet 은 대부분 다음날 크론에서 자가회복(최근 게시 대부분 08-02), 실제로 밀린 건 `linux-arch-platform` 하나뿐 — "매일 뭔가 실패"의 체감과 달리 결손은 하루 단위로 자가치유되고 있었다. 6일 7건 실패를 3부류로 나눠 구조적 대응(위 cron-관찰 엔트리들과 달리 **실 코드 변경** 세션). (1) **evidence.kind soft-fail** — 07-25 `"repo"` band-aid 가 08-02 `"patch"` 로 재발(값 추가 whack-a-mole 실증) → `research-runner.mjs` 의 `normalizeDossier` 에서 미지 kind→`'other'` 강등으로 어떤 값도 topic drop 안 되게 비치명화(enum 에 patch/release/other 추가는 라벨 보존용). 상세·닫힌 루프는 [[dossier-evidence-kind-enum-reject]]. (2) **CJK 필터 일반화** — `opensource-curation` discover 에만 있던 비한글 CJK 레포 필터를 `scripts/lib/foreign-cjk.mjs` 로 추출해 형제 토픽 `opensource`(트렌딩) `draft-opensource.mjs` 에도 적용(07-30 중국어 "本机回环" 혼입 → quality-guard 차단 원인 제거). [[llm-content-quality-guards]]. (3) **실패 토픽 자동 1회 재시도** — research 타임아웃/`claude exited with 1`/research non-JSON 같은 외부의존 transient(12토픽×외부호출의 불가피한 배경 실패율, 05-16 기수록 "매일 1개씩 fail" 구조·[[shell-set-eu-topic-isolation]] 격리의 연장)를 `daily-deploy.sh` 의 `retry_once`(20초 후 1회 재시도)로 자동 회복. 격리(`if!`/continuing) 유지, lens 루프는 `env PUBLISH_DAILY=1 …` 로 전달(var-prefix 를 `"$@"` 로 넘기면 리터럴 인자화되는 POSIX 함정 회피). 07-27 도입한 15분 research 타임아웃도 08-01 opensource 가 초과 → 재시도가 커버하는 구조. **복구**: `linux-arch-platform` 08-02 를 저장된 `research-stdout-2026-08-02.txt` 를 `RESEARCH_RAW_PATH` 로 재사용(kind 강등 수정 후 검증 통과, API 재호출 없음)→rewrite→publish. 테스트 128/128, build 12토픽·939포스트, 비한글 CJK 0. **핵심 교훈**: per-cause 개별 수정은 whack-a-mole(kind repo→patch→…, CJK 토픽별 확산)이고 다부류 fleet 의 transient tail 은 0 이 안 된다 — 격리 + 미지값 soft-fail + 자동재시도의 **구조적 대응**이 근본책. 부류 1·2(결정론적)는 닫히고, 부류 3(transient)은 자동재시도로 same-day 결손을 줄이되 드물게 이중 실패 시 다음날 자가회복에 의존. (출처: dev-blog commits 1608b20·239c5b7, 2026-08-02 세션)
- 2026-08-03 (03:00~05:00 새벽 cron 26건 — **retry_once 첫 cron 실전 발화** + 교정 재시도 3번째 실측 + anti-bot 12번째 관측): Research Dossier 12건 + Newsletter Write 13건 + Weekly Digest 1건. **운영 관찰**: (1) **08-02 도입 `retry_once`(commit `1608b20`)가 도입 후 첫 새벽 cron 에서 바로 발화** — Opensource Trending 토픽이 R#1(03:19 `1d25`) → W#1(03:24 `861a`) → W#2 교정 재시도(03:28 `ae2b`, `[재시도]` 지시문 부착) → **R#2(03:31 `36d0`, 동일 collect 후보 payload 15건 재실행)** → W#3(03:37 `e697`, 새 dossier `generatedAt` 18:31Z 수신) 순으로 5세션 체인을 남김. 어댑터 내부 교정 재시도(e28df77)가 실패하자 topic 단위 retry_once 가 research 부터 전체 재실행하는 **2계층 재시도의 연쇄**가 처음 실측됨. 교정 재시도는 07-29·07-30 에 이어 3번째 실측이며 **3회 전부 Opensource Trending write** — 토픽 상관 가설이 표본 3회로 강화(상세 [[llm-json-parse-retry-with-dump]]). 최종 게시 성공 여부는 헤드리스 로거 특성상 로그 미기록. (2) **anti-bot 12번째 관측, 신규 변형 없음** — Linux Daily research(`054b`)가 유일한 1턴 응답으로 "모든 대상 URL이 Anubis 봇 차단에 걸렸습니다. 대체 경로로 재시도합니다"만 기록 — 판정 동일이라 [[newsletter-research-anti-bot-blocking]] 본문 무변경(08-02 선례와 동일 처리). Lens research 들(`9660`·`296e`·`386f`)의 payload 에 collect 선캡처된 UA 스윕 실측(curl 기본 403 / git·lei 200 / Mozilla 200+Anubis)도 기존 판정과 일치. (3) **로그 소실 패턴 지속** — research 12건 중 `054b` 만 1턴, write 13건·Weekly Digest 전건 `assistant_turns: 0` — 07-20 이래 기수록 "헤드리스 로거 산출 소실" 프레임 내 수준. (4) 전날 저녁 20:55 의 단독 Lens write 로그(`56cb`, 0턴)는 08-02 `239c5b7` arch-platform 수동 복구 세션의 부산물로 그 세션 자체는 08-02 wiki-ingest 로 기수집. 뉴스 콘텐츠는 기존 결정대로 전량 스킵. **코드 변경 없음** (출처: session-logs/20260803-03*, 20260803-04*, -050031-551b-*, 20260802-205513-56cb-* 사이클 27건)
