...
- `--clone` 은 config / .env / SOUL / skills 만 복사, **메모리·세션은 fresh** — 단 **실측(2026-07-04)과 어긋나는 사례 있음**: `--clone-from maccoder` 로 만든 역할 프로필들이 maccoder 의 `memories/MEMORY.md`(에 오염 복사돼 있던 default 의 "맥비" 정체성)를 물려받아 전부 자기를 맥비로 인식했다. 버전·경로에 따라 memory 가 따라올 수 있으니 클론 후 MEMORY.md 확인 필요. 교훈: **페르소나 정체성은 가변·전파되는 memory 가 아니라 영속 페르소나 파일(SOUL)에 둬야** 클론 오염이 없다 ([[multi-profile-cli-agent-isolation]] §정체성).
- **default 프로필은 폴더가 없다** — `~/.hermes/` 루트 자체(SOUL.md / config.yaml / .env / memories/ / kanban.db)가 default 이며, 그 `.env` 는 전 프로필 글로벌 공유다 (`WIKI_PATH` 등 — 공유 위키의 동시 쓰기 위험은 [[multi-agent-shared-wiki-concurrency]]).
...
- **위임 이벤트의 실제 형태 (2026-07-04 라이브 캡처)**: 문서(PHASE0)와 코드(jobs-api.ts) 어휘가 둘 다 실물과 달랐다. 실제는 `{event: "tool.started"/"tool.completed", tool: "delegate_task", duration, preview}` — **필드명이 `tool` 이며 input/output 필드가 없다**. 위임 내용·결과는 tool 이벤트가 아니라 리더의 `message.delta` / `run.completed.output` 으로 온다. 교훈: 미문서화 이벤트 스트림은 방어적 파서를 선구현해 스펙 오류를 조기 노출하되, **최종 판정은 반드시 실제 캡처 페이로드로** — "권위 있는" 코드 어휘조차 실물과 달랐다.
- **`API_SERVER_KEY` 는 세션 연속(continuation)에 사실상 필수**: 루프백 무인증 구성이라도 세션 연속 기능만은 키 없으면 403. 앱↔게이트웨이(HERMES_API_TOKEN/API_SERVER_KEY)와 게이트웨이↔모델(예: Codex 토큰)은 **서로 다른 층의 2단계 인증**이다.
- **`state.db` 에 `messages_fts`(FTS5) 존재**: 게이트웨이가 메시지 검색 API 를 안 줘도, 프로필 `state.db` 를 readonly 로 직접 열어 FTS 단일 쿼리로 전문 검색이 가능하다 (`created_at` 초→ms 변환 주의).
...
**방식 B 의 dispatcher 주의 (2026-07-04 실측)**: 칸반 dispatch 는 게이트웨이 설정 `dispatch_in_gateway`(기본 60초 주기, `dispatch_interval_seconds`) 로 켜지는데, **여러 게이트웨이가 동시에 켜져 있으면 같은 태스크를 서로 claim·회수·재spawn 하는 churn** 으로 파이프라인이 진행되지 않는다. 보드당 dispatcher 는 하나만 (default 만 true, 나머지 false) → 상세와 일반화는 [[single-dispatcher-per-queue]]. 워커는 on-demand spawn 이라 역할별 게이트웨이 상시 기동은 불필요. 오케스트레이션 방식의 일반 분류는 [[multi-agent-orchestration-taxonomy]].
...
- 2026-07-05: 멀티에이전트 운영 실험 실측 반영 — ① 위임 이벤트 실제 형태(`tool.started/completed`, input/output 없음, 결과는 message.delta 로) 라이브 캡처 정정 ② `API_SERVER_KEY` = 세션 연속 필수 + 2단계 인증 층 구분 ③ `state.db` `messages_fts`(FTS5) ④ 칸반 dispatcher 단일화 규칙([[single-dispatcher-per-queue]]) ⑤ `--clone` 의 memory 전파 실측 반례 + 정체성은 SOUL 에 ⑥ default 프로필 = `~/.hermes/` 루트 + `.env` 글로벌 공유 (출처: session-logs/20260704-132738-e509-*)
